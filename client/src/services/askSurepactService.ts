...
export function processAskSurePactQuery(
  ...
  // 8. Projects & Capital Works / Department Spend Query
...
function getFallbackGrants() {
  return [
    ...
    },
    {
      ...
      categoryActualSpent: { Equipment: 450000, Training: 40000, Travel: 20000 }, // 12.5% equipment variance breach
...
export interface FunderAnalytics {
  ...
  avgRoiPerHour: number;
  avgDrawdownDays: number;
  acquittalFriction: 'Low' | 'Medium' | 'High';
  difficultyRating: 'Easy' | 'Moderate' | 'Challenging' | 'Highly Competitive';
}

export function getFunderAnalytics(funderName?: string): FunderAnalytics {
  const name = (funderName || '').toLowerCase();
  if (name.includes('territory families') || name.includes('nt dept')) {
    return {
      funderName: funderName || 'NT Dept of Territory Families',
      winRatePercent: 75,
      totalSubmitted: 4,
      totalAwardedAmount: 970000,
      avgRoiPerHour: 35000,
      avgDrawdownDays: 14,
      acquittalFriction: 'Low',
      difficultyRating: 'Moderate'
    };
  }
  if (name.includes('phn') || name.includes('health')) {
    return {
      funderName: funderName || 'Northern Territory PHN',
      winRatePercent: 80,
      totalSubmitted: 5,
      totalAwardedAmount: 1450000,
      avgRoiPerHour: 42000,
      avgDrawdownDays: 18,
      acquittalFriction: 'Medium',
      difficultyRating: 'Easy'
    };
  }
  if (name.includes('infrastructure') || name.includes('federal')) {
    return {
      funderName: funderName || 'Federal Dept of Infrastructure',
      winRatePercent: 60,
      totalSubmitted: 3,
      totalAwardedAmount: 3250000,
      avgRoiPerHour: 68000,
      avgDrawdownDays: 21,
      acquittalFriction: 'Medium',
  ...
  return {
    funderName: funderName || 'Government Funding Agency',
    winRatePercent: 68,
    totalSubmitted: 6,
    totalAwardedAmount: 1200000,
    avgRoiPerHour: 31000,
    avgDrawdownDays: 16,
    acquittalFriction: 'Low',
    difficultyRating: 'Moderate'
